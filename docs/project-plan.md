...
- 방향 벡터는 긴 기준선인 `rightWrist - rightShoulder`를 사용한다.
- 팔꿈치는 두 팔 구간 길이와 비율, 팔꿈치 각도 검사에 사용한다.
- 정규화 포인터는 손목에서 어깨→손목 벡터를 기본 `0.25`배 연장해 계산한다.
- 포인터 후보는 `[0.0, 1.0]`로 clamp하고 이미지 y축은 Python에서 뒤집지 않는다.
- 관절 좌표에는 EMA `0.35`를 적용하고 유효 포인팅 2프레임 후 활성화한다.
- `PARTIAL`, `LOST`와 기하 실패는 즉시 포인터를 비활성화하고 안정화 상태를
  초기화한다.
...
| 기본 visibility `0.5`가 모든 조명·거리에서 적합하지 않을 수 있음 | 설정으로 유지하고 전체 오른팔이 보이는 실제 샘플로 조정 |
